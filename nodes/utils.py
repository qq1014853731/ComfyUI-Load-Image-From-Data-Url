...
class ImageNodeUtils:
    ...
    def read_limited_stream(
        ...
        max_bytes = self.normalize_max_download_bytes(max_download_bytes)
        if max_bytes is not None and content_length is not None and content_length > max_bytes:
            raise ValueError(
                f"{source_label} is too large: {self.format_bytes(content_length)} "
                f"exceeds limit {self.format_bytes(max_bytes)}"
            )
        ...
        while True:
            ...
            if max_bytes is not None and total > max_bytes:
                raise ValueError(
                    f"{source_label} is too large: exceeds limit {self.format_bytes(max_bytes)}"
                )
            chunks.append(chunk)
        return b"".join(chunks)

    def read_uri(self, uri: str, timeout: int, max_download_bytes: int) -> bytes:
        if uri.startswith("data:"):
            return self.read_data_url(uri)
        ...
        if scheme == "file":
            return self.read_file_url(parsed)
        if scheme == "s3":
            return self.read_s3_url(parsed, timeout=timeout, max_download_bytes=max_download_bytes)
        if scheme in ("http", "https", "ftp"):
            return self.read_remote_url(uri, timeout=timeout, max_download_bytes=max_download_bytes)
        if scheme == "":
            return self.read_local_file(uri)
    ...
    def read_file_url(self, parsed: urllib.parse.ParseResult) -> bytes:
    ...
    def read_s3_url(
        ...
        try:
            ...
            with contextlib.closing(response["Body"]) as body:
                return self.read_limited_stream(
                    body,
    ...
    def read_remote_url(self, url: str, timeout: int, max_download_bytes: int) -> bytes:
    ...
    def pad_comfy_tensors(image_tensor: torch.Tensor, mask_tensor: torch.Tensor, height: int, width: int):
        ...
        if pad_h < 0 or pad_w < 0:
            raise ValueError(
                f"Cannot pad tensor from {(current_h, current_w)} to smaller target {(height, width)}."
            )
