...
这个节点用于按条件旁路上游分支，并在不触发该分支执行的情况下直接输出 `None`。

- `enabled`：
  - `true`：请求 lazy 输入 `value` 并原样透传
  - `false`：不请求 `value`，直接输出 `None`
- `value`：
  - 可接收任意 ComfyUI 类型（`*`）
  - 这是 lazy 输入，只有在 `enabled = true` 时才会被求值

适合：
- 跳过不接受 `None` 的中间节点
- 把 `None` 传给支持可选输入的下游节点
- 构建禁用分支不执行的条件流程

### Preview Image (With None)

这个节点与 ComfyUI 的预览节点类似，但可安全接收 `None`。

- `image` 输入是可选的。
- 当 `image` 为 `None` 时，该节点不执行预览，也不会报错。
- 当 `image` 有值时，行为与普通预览节点一致，会正常预览/保存预览图。
...
![Lazy Gate 工作流示例](./docs/example.png)
